...
export default function Forecast({ weatherForecast, currentWeather }) {
  ...
  const max = daysArray.map((item) => {
    return Math.round(item.max);
  });
  const min = daysArray.map((item) => {
    return Math.round(item.min);
  });
  ...
  return (
    <div id="forecast-box">
      {daysArray.map((item, index) => {
        return (
          <ForecastDay
            parametersLine={tempCoordiantes.line[index]}
            parametersBox={tempCoordiantes.box[index]}
            index={index}
            key={index}
